...
class IriTest(unittest.TestCase):
    ...
    def test_is_blank_node(self):
        ...
        alice = lp_model.new(name=alice_name, mboxes={alice_mail}, short_bio_en=alice_bio_en)
        ...
        key = gpg_model.create(fingerprint="deadbeef", hex_id="deadbeef")
        ...
        with self.assertRaises(Exception):
            lp_model.new(iri="http://localhost/.well-known/genid/2387335")
    ...
    def test_iri_uniqueness(self):
        ...
        with self.assertRaises(OMUniquenessError):
            lp_model.create(iri=bob_iri, name=bob_name, mboxes=bob_emails, short_bio_en=u"Will not exist")

        # Forces the creation (by claiming your are not)
        # Dangerous!
        short_bio_en = u"Is forced to exist"
        bob2 = lp_model.create(iri=bob_iri, name=bob_name, mboxes=bob_emails, short_bio_en=short_bio_en, is_new=False)
