...
class FindTest(unittest.TestCase):
    ...
    def test_filter_hashless_iri_types_and_names(self):
        ...
        self.assertEquals({bob.id, alice.id, key.id}, {r.id for r in manager.filter(hashless_iri=doc_iri)})
        self.assertEquals({bob.id, alice.id}, {r.id for r in manager.filter(hashless_iri=doc_iri,
                                                                            types=[MY_VOC + "LocalPerson"])})
        # Missing type (name is thus ambiguous)
        with self.assertRaises(OMAttributeAccessError):
            manager.filter(hashless_iri=doc_iri, name=alice_name)
    ...
    def test_get_hashless_iri_types_and_names(self):
        ...
        self.assertEquals(document.id, manager.get(hashless_iri=doc_iri).id)
        self.assertEquals(bob.id, manager.get(hashless_iri=doc_iri, types=[MY_VOC + "LocalPerson"]).id)
        self.assertEquals(key.id, manager.get(hashless_iri=doc_iri, types=[MY_VOC + "LocalGPGPublicKey"]).id)
